      ...
      if (cur[type].hasOwnProperty(GSIM.REPRESENTED_VARIABLE)) {
        ...
        if (rv.hasOwnProperty(GSIM.VARIABLE)) {
          ...
          lineage.nodes.push({
            ...
            symbolType: variableSymbols.variable,
            nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, rv_v[GSIM.NAME]),
            nodeLabelDescription: daplaJsUtilities.getLocalizedGsimObjectText(language, rv_v[GSIM.DESCRIPTION])
          });
          lineage.links.push({
            source: rv_v.id,
            target: rv.id
