...
const nodeGraphFromDataset = (id, data, language) => data.reduce((acc, cur) => {
  ...
  const lineageFields = daplaJsUtilities.getNestedObject(cur, ['unitDataSet', 'lineage', 'reverseLineageFieldLineageDataset']);
          ...
          if (smart !== null) {
            if (smart.hasOwnProperty('instanceVariable') && smart.instanceVariable !== null) {
              lineage.nodes.push({
                ...
                nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, smart.instanceVariable.name),
              ...
              if (smart.instanceVariable.hasOwnProperty('representedVariable')) {
                lineage.nodes.push({
                  ...
                  nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, smart.instanceVariable.representedVariable.name),
                ...
                if (smart.instanceVariable.representedVariable.hasOwnProperty('variable')) {
                  lineage.nodes.push({
                    ...
                    nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, smart.instanceVariable.representedVariable.variable.name),
              ...
              lineage.nodes.push({
                id: smart.id,
                nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, smart.name),
...
function LineageView({
    ...
    if (!loading && !error && data !== undefined) {
      if (dataType !== 'dataset') {
        ...
        const graphNodes = {
          nodes: nodesFrom.nodes.concat([{
            id: dataId,
            nodeLabelName: daplaJsUtilities.getLocalizedGsimObjectText(language, data[0].unitDataSet.name),
            color: hash$1.hex(dataId),
            size: 1000,
            fontSize: 20,
            highlightFontSize: 20,
  ...
  return /*#__PURE__*/React__default['default'].createElement(React__default['default'].Fragment, null, /*#__PURE__*/React__default['default'].createElement(semanticUiReact.Header, {
    content: `Sporing av ${dataId} (${dataType})`
